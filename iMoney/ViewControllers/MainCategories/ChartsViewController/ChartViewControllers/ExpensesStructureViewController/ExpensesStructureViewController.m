//
//  ExpensesStructureViewController.m
//  iMoney
//
...
@interface ExpensesStructureViewController ()
...
@implementation ExpensesStructureViewController

- (void)viewDidLoad {
    
    [super viewDidLoad];
    self.title = @"Expence structure";
...
- (void)setupPieChart {
    ...
    self.expencePieChart = [[PieChart alloc] initWithFrame:self.supportView.bounds];
    [self.expencePieChart setDataSource:self];
    [self.expencePieChart setDelegate:self];
    [self.expencePieChart setLegendViewType:LegendTypeHorizontal];
    [self.expencePieChart setShowCustomMarkerView:TRUE];
    [self.expencePieChart drawPieChart];
    self.expencePieChart.showValueOnPieSlice = NO;
    [self.supportView addSubview:self.expencePieChart];
}
...
- (void)generatePlotValues:(NSArray <Transaction *> *)tranasactions {
    ...
    [tranasactions enumerateObjectsUsingBlock:^(Transaction * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.transactionType == kTransactionTypeExpense) {
    ...
    [self.expencePieChart reloadPieChart];
}
