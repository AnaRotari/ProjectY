...
- (void)setup
{
    ...
    self.toolbar.backgroundColor = RGBColor(42, 3, 70, 1);
...
- (void)setInitialItemAtIndex:(NSUInteger)index
{
    if (index < self.dataSourceForCustomPickerType.count) {
        self.indexOfSelectedRowInPicker = index;
...
- (IBAction)doneButtonAction:(UIButton *)sender
{
        ...
        self.pickerType == DatePickerType)
    {
        ...
        [self.delegate didSelectDate:self date:self.datePicker.date formattedString:[NSString stringWithFormat:@"%@", [dateFormatter stringFromDate:self.datePicker.date]]];
    } else
        
        // * CustomPickerType
        if (self.pickerType == CustomPickerType) {
            [self.delegate didSelectItemAtIndex:self index:self.indexOfSelectedRowInPicker];
        }
